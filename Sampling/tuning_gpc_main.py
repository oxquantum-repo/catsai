...
def update_gpc(gpc_dict, points_extended, poff_extended, pks_all, lowres_score, M=None):
    ...
    gpc_dict['valid'].create_model(points_extended, poff_extended.astype(float))
    ...
    if len(np.unique(pks_poff)) < 2:
        return
    gpc_dict['peak'].create_model(points_poff, pks_poff.astype(float))
    ...
    if len(np.unique(gt_threshold)) < 2:
        return
    gpc_dict['goodscore'].create_model(points_peaks, gt_threshold.astype(float))
    '''
