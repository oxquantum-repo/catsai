...
def show_dummy_device(device,configs,save=True):
    ...
    dev_vol = utils.extract_volume_from_mock_device(conf_g['lb_box'],conf_g['ub_box'],10,device)
    ax = utils.plot_volume(dev_vol,conf_g['lb_box'],conf_g['ub_box'],conf_g['ub_box'],10)
...
def show_gpr_gpc(gpr,configs,points,condidx,origin,gpc=None,save=True):
    conf_g = configs['general']
    print("Plotting gpr")
    gpr_vol = utils.extract_volume_from_gpr(conf_g['lb_box'],conf_g['ub_box'],50,gpr)
    ax = utils.plot_volume(gpr_vol,conf_g['lb_box'],conf_g['ub_box'],origin,50,cmap_func=gpc)
